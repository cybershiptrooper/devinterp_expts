...
class SGLD(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        self.restrict_to_orth_grad = restrict_to_orth_grad
    ...
    def step(self, closure=None):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # if self.restrict_to_orth_grad:
                #     w0 = torch.nn.utils.parameters_to_vector([p.data]).detach().clone()
                    # # get gradient as vector
                    # g = torch.nn.utils.parameters_to_vector([p.grad]).detach().clone()
                    # # normalize gradient
                    # g = g / g.norm(p=2)
                ...
                # Restrict to orthogonal gradient
                # if self.restrict_to_orth_grad:
                #     print(p.data.shape, w0.shape)
                #     w1 = torch.nn.utils.parameters_to_vector([p.data]).detach().clone()
                #     diff = (w1 - w0).dot(w0) / w0.dot(w0) * w0
                #     p.data.add_(diff)       
